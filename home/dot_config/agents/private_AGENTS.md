...
- Before implementation, check factual claims in prompts, tickets, MR/PR descriptions, chats, and docs against code and data. For wrong claims or better approaches, show file:line, a failing case, or measured cost. Propose the alternative and ask: proceed anyway, or take the alternative? Otherwise proceed.
...
- Bind application, preview, and artifact HTTP servers to `0.0.0.0`. Report `http://<lan-ip>:<port>`, never `localhost`. Get the LAN IP with `ipconfig getifaddr en0` (macOS), `hostname -I` (Linux), or `ipconfig` (Windows). Serve visual output this way, including screenshots, rendered pages, diagrams, and reports; never use local-file links. Tool-control endpoints keep their required binding.
- Give each dev server and worktree its own open `PORT` and export it. At task end, close MCP resources, stop servers and background processes you started, and free their ports.
- At task end, assess helper scripts/tools for reuse. If reusable by a teammate or later task, propose a project path and let me decide. Do not leave them in the repository without consent.
- Remove temporary scratch files, helpers, test data, extra branches, debug output, and logs when the task ends. Keep requested files and copy files awaiting use.
- Make copyable commands, comments, commit messages, and snippets safe for terminals wrapping at about 80 characters:
  - Show every proposal's exact final wording in chat, even with a copy file. Put other longer commands in `~/.cache/agents/copy/run-<task>.sh` and text in `~/.cache/agents/copy/<task>.md` (see Directories).
...
- Skill code blocks use POSIX `sh`. Use Git Bash on Windows; translate to PowerShell only when Git Bash is unavailable. Keep every git flag unchanged.

## Directories

Never write to OS temporary directories (`/tmp`, `$TMPDIR`, `%TEMP%`) or harness scratchpads, even when the harness instructs it. Use these paths and create missing parents. Follow XDG; default to `~/.cache` and `~/.local/state`, including under the Windows user profile.

- Scratch: `$XDG_CACHE_HOME/agents/scratch/<task>/` (default `~/.cache/agents/scratch/`). Disposable intermediate results, command logs, extracted packages, and helper scripts.
- Copy: `$XDG_CACHE_HOME/agents/copy/` (default `~/.cache/agents/copy/`). Only `run-<task>.sh` and `<task>.md` for me to run or `copy`. Follow General's cleanup rule.
- Backups: `$XDG_STATE_HOME/agents/backups/<repo>/<YYYYMMDD-HHMM>-<reason>/` (default `~/.local/state/agents/backups/`). Files copied before force-removal, overwrite, or migration, with relative paths preserved. Never auto-prune. Report the backup path.
- Keep files at fixed paths when tools require them, such as repository-root linter configuration.

## Skills own the detail

Load each skill before its first relevant action. It owns the detailed rules.

- Sub-agents: `split-task` owns splitting; apply its thresholds automatically. `sub-agents` owns roles, pins, overrides, prompts, monitoring, handoffs, escalation, and result checks. Keep judgement, integration, and the final check in the main session.
...
- Start with the answer; omit preambles and repeated closing summaries. State uncertainty as fact. Never invent unchecked specifics, such as versions, dates, flags, or line numbers. Name the command or file that settles them.
