...
- Get my explicit consent before you change a repository's `.git/config`. This includes `git config --local` writes and fixes suggested by a question about the current settings. Reading the config does not need consent.
