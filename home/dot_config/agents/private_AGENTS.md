...
- Give each dev server and worktree its own random free port. Use that same port for the whole task; pass it to each start command. At task end, close MCP resources, stop servers and background processes you started, and free their ports.
- At task end, remove scratch files, helpers, test data, extra branches, debug output, and logs. Keep requested files and copy files awaiting use. If a helper could serve a teammate or a later task, propose a project path and let me decide; never leave it in the repository without consent.
...
- Copy: `~/.cache/agents/copy/`. Only `run-<task>.sh` and `<task>.md` for me to run or `copy`. Follow General's cleanup rule.
...
Load each skill before its first relevant action; it owns the detailed rules. If the harness does not list it, read `~/.config/agents/skills/<name>/SKILL.md`.
