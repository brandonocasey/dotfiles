...
When checks are pending and the user asks to watch, load the `sub-agents`
skill and start one `cheap` background watcher for the repository. Give it the current pull request numbers, source SHAs, and
URLs. It must:
